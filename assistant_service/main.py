...
class AssistantEngineAPI:
    ...
    async def _iterate_run_events(self, thread_id: str, human_query: str) -> AsyncGenerator[Any, None]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=502, detail=f"Failed to create message (correlation_id: {correlation_id[:8]})"
            ) from err
            ...
            raise HTTPException(
                status_code=500, detail=f"Internal server error (correlation_id: {correlation_id[:8]})"
            ) from err
        ...
        try:
            ...
            raise HTTPException(
                status_code=502, detail=f"Failed to create run (correlation_id: {correlation_id[:8]})"
            ) from err
        except Exception as err:  # noqa: BLE001
            logger.error(
        ...
        async for event in self._iterate_run_events(thread_id, human_query):
            if event.event == "thread.run.step.completed" and event.data.step_details.type == "message_creation":
                ...
                try:
                    ...
                    raise HTTPException(
                        status_code=502, detail=f"Failed to retrieve message (correlation_id: {correlation_id[:8]})"
                    ) from err
                except Exception as err:  # noqa: BLE001
                    logger.error(
    ...
    async def start_endpoint(self) -> dict[str, str]:
        ...
        with CorrelationContext() as correlation_id:
            try:
                ...
                raise HTTPException(
                    status_code=502, detail=f"Failed to start thread (correlation_id: {correlation_id[:8]})"
                ) from err
            except Exception as err:  # noqa: BLE001
                logger.error(
    ...
    async def chat_endpoint(self, request: ChatRequest) -> ChatResponse:
        ...
        with CorrelationContext() as correlation_id:
            if not request.thread_id:
                logger.error("Missing thread_id in chat request", correlation_id=correlation_id)
                raise HTTPException(status_code=400, detail=f"Missing thread_id (correlation_id: {correlation_id[:8]})")
    ...
    async def stream_endpoint(self, websocket: WebSocket) -> None:
        ...
        try:
            # Keep connection open for multiple message exchanges
            while True:
                with CorrelationContext() as correlation_id:
                    try:
                            ...
                            async for event in self._process_run_stream(thread_id, message):
                                try:
                                    ...
                                    if self._is_websocket_disconnect(err):
                                        ...
                                        await self._send_websocket_error(
                                            websocket, "Failed to send event", "send_error"
                                        )
                                        break  # Break inner loop, continue to next message

                            logger.info(
                                "WebSocket stream completed", thread_id=thread_id, correlation_id=correlation_id
                            )
                        ...
                        # Continue to next message instead of breaking connection
                        continue
        except Exception as err:  # noqa: BLE001
            logger.error(
