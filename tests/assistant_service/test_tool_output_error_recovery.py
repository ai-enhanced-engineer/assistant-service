...
async def test_submit_tool_outputs_success():
    ...
    result = await submit_tool_outputs_with_backoff(
        mock_client, "thread_123", "run_456", tool_outputs
    )
...
async def test_submit_tool_outputs_retry_then_success():
    ...
    mock_client.beta.threads.runs.submit_tool_outputs.side_effect = [
    ...
    result = await submit_tool_outputs_with_backoff(
        mock_client, "thread_123", "run_456", tool_outputs, retries=2
    )
    ...
    class DummyConfigRepo:
        ...
        def read_config(self):
            return EngineAssistantConfig(
                assistant_id="a", assistant_name="name", initial_message="hi"
            )
        ...
        with patch("assistant_service.main.cancel_run_safely", return_value=True) as mock_cancel:
            ...
            async def mock_events():
                ...
                yield types.SimpleNamespace(
                    ...
                    data=types.SimpleNamespace(id="run_123")
                )
                ...
                yield types.SimpleNamespace(
                    ...
                    data=types.SimpleNamespace(step_details=step_details)
                )
                ...
                yield types.SimpleNamespace(
                    ...
                    data=types.SimpleNamespace(required_action=required_action)
                )
